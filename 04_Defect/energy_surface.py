...
plot_coord_full = np.mgrid[0:0.5:nx*1j, 0:0.5:ny*1j, 0:1:nz*1j]
...
V = gd(np.concatenate([data_coord + shift for shift in shifts]), np.concatenate([data_value for _ in shifts]), np.transpose(plot_coord_full, axes = [1, 2, 3, 0]), method = 'nearest')
V = np.exp(-(V-np.min(V))/T)

xs, ys, zs = plot_coord_full
vs = V
