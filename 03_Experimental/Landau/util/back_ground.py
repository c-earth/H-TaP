...
def subbg_po(Ts, Bs, MRs, po_power, T_max, resu_dir):
    ...
    for T, MR in zip(Ts, MRs):
        ...
        ax[1].set_xlabel(r'$B$ [T]', fontsize = 30)
        ax[1].yaxis.get_offset_text().set_size(26)
        ax[1].figure.canvas.draw()
        ax[1].set_ylabel(r'$\Delta$ MR'+ f'[%]', fontsize = 30)
...
def subbg_pp(Ts, Bs, MRs, pp_power, pieces, T_max, resu_dir):
    ...
    for T, MR in zip(Ts, MRs):
        ...
        f, ax = plt.subplots(2, 1, figsize = (8,7), sharex = True)
        ...
        ax[0].yaxis.get_offset_text().set_size(26)
        ax[0].figure.canvas.draw()
        ax[0].set_ylabel('MR'+ f'[%]', fontsize = 30)
        ax[0].legend(fontsize = 24)
...
def subbg_de(Ts, Bs, MRs, avg_window, T_max, resu_dir):
    ...
    for T, MR in zip(Ts, MRs):
        ...
        ax[1].yaxis.get_offset_text().set_size(26)
        ax[1].figure.canvas.draw()
        ax[1].set_ylabel(r'$\partial^2$MR/$\partial B^2$'+ f'[%]', fontsize = 30)
        ax[1].legend(fontsize = 24)
        f.tight_layout()
