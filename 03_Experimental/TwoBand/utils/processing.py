...
def plot_residual(savename, params_files):
    ...
    for params_file in params_files:
        ...
        ax1.plot(params[:, 0], params[:, -4]*1E-2, linestyle='-', marker='o', markersize=10, linewidth=3, label=name, color = '#2F349A')
        ax2.plot(params[:, 0], params[:, -3]*1E-2, linestyle='-', marker='o', markersize=10, linewidth=3, label=name, color = '#2F349A')

    ax1.set_xlabel(r'$T$ [K]', fontsize=30)
    ax1.set_ylabel(r'RMSE of Fitted $\sigma_{xx}$ [$\Omega^{-1}$cm$^{-1}$]', fontsize=30)
    ax1.tick_params(axis='both', which='both', direction='in', labelsize=26, width=1.5, length=5)
    ax1.xaxis.get_offset_text().set_size(26)
    ax1.yaxis.get_offset_text().set_size(26)
    ...
    ax2.set_ylabel(r'RMSE of Fitted $\sigma_{xy}$ [$\Omega^{-1}$cm$^{-1}$]', fontsize=30)
    ax2.tick_params(axis='both', which='both', direction='in', labelsize=26, width=1.5, length=5)
    ax2.xaxis.get_offset_text().set_size(26)
    ax2.yaxis.get_offset_text().set_size(26)
...
def plot_fitting_params(savename, params_files):
    for params_file in params_files:
        ...
        ax1.plot(params[:, 0], params[:, 7]*1E-6,linestyle='--',marker='o',markersize=10,linewidth=3,label='$n_e$', color = '#E33119')
        ax1.plot(params[:, 0], params[:, 3]*1E-6,linestyle='--',marker='o',markersize=10,linewidth=3,label='$n_h$', color = '#2F349A')
        ax2.plot(params[:, 0], params[:, 5]*1E4,linestyle='--',marker='s',markersize=10,linewidth=3,label='$\mu_e$', color = '#E33119')
        ax2.plot(params[:, 0], params[:, 1]*1E4,linestyle='--',marker='s',markersize=10,linewidth=3,label='$\mu_h$', color = '#2F349A')

        ax1.set_xlabel(r'$T$ [K]', fontsize=30)
        ax1.set_ylabel(r'$n$ [cm$^{-3}$]', fontsize=30)
        ...
        ax2.legend(fontsize=30, loc='center right')
        ax2.tick_params(axis='both', which='both', direction='in', labelsize=26, width=1.5, length=5)
        ax2.ticklabel_format(axis = 'y', style = 'sci', scilimits = (0, 0), useMathText = True)
        ax2.xaxis.get_offset_text().set_size(26)
        ax2.yaxis.get_offset_text().set_size(26)
...
def plot_sigma(Ts, Bxxs, Sxxs, Bxys, Sxys, resu_dir, S_scale):
    ...
    ax1.set_xlabel(r'$B$ [T]', fontsize = 30)
    ax1.set_ylabel(r'$\sigma_{xx}$ [$\Omega^{-1}$ cm$^{-1}$]', fontsize = 30)
    ax1.tick_params(which = 'both', direction = 'in', top = False, right = False, length = 5, width = 1.5, labelsize = 26)
    ...
    ax2.set_xlabel(r'$B$ [T]', fontsize = 30)
    ax2.set_ylabel(r'$\sigma_{xy}$ [$\Omega^{-1}$ cm$^{-1}$]', fontsize = 30)
    ax2.tick_params(which = 'both', direction = 'in', top = False, right = False, length = 5, width = 1.5, labelsize = 26)
    ...
    for T, Bxx, Sxx, Bxy, Sxy in zip(np.flip(Ts, axis = 0), np.flip(Bxxs, axis = 0), np.flip(Sxxs, axis = 0), np.flip(Bxys, axis = 0), np.flip(Sxys, axis = 0)):
        rhoxx = Sxx/(Sxx**2 + Sxy**2)
        ax.plot(Bxx, rhoxx / S_scale / 1E-2, '-', color = colors[int(T)-2], linewidth = 3)
    ...
    ax.set_xlim((np.min(Bxx), np.max(Bxx)))
    ...
    f.savefig(os.path.join(resu_dir, 'rhoxx_vs_B.png'))
    plt.close()


    f, ax = plt.subplots(1, 1, figsize=(8,7))
    for T, Bxx, Sxx, Bxy, Sxy in zip(np.flip(Ts, axis = 0), np.flip(Bxxs, axis = 0), np.flip(Sxxs, axis = 0), np.flip(Bxys, axis = 0), np.flip(Sxys, axis = 0)):
        rhoxy = Sxy/(Sxx**2 + Sxy**2)
        ax.plot(Bxx, rhoxy / S_scale / 1E-2, '-', color = colors[int(T)-2], linewidth = 3)
    ...
    f.savefig(os.path.join(resu_dir, 'rhoxy_vs_B.png'))
    plt.close()
...
def plot_relative_residual(savename, params_files):
    ...
    for params_file in params_files:
        params = read_params(params_file)
        name = params_file.split('/')[-1][:-11]
        ax1.plot(params[:, 0], params[:, -2], linestyle='-', marker='o', markersize=10, linewidth=3, label=name, color = '#2F349A')
        ax2.plot(params[:, 0], params[:, -1], linestyle='-', marker='o', markersize=10, linewidth=3, label=name, color = '#2F349A')

    ax1.set_xlabel(r'$T$ [K]', fontsize=30)
    ax1.set_ylabel(r'RMSRE of Fitted $\sigma_{xx}$ []', fontsize=30)
    ax1.tick_params(axis='both', which='both', direction='in', labelsize=26, width=1.5, length=5)
    ax1.xaxis.get_offset_text().set_size(26)
    ax1.yaxis.get_offset_text().set_size(26)
    ax1.ticklabel_format(axis = 'y', style = 'sci', scilimits = (0, 0), useMathText = True)
